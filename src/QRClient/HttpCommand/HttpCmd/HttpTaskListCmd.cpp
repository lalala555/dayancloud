...
HttpTaskListCmd::HttpTaskListCmd(const QString& timeFilter, int showType, int pageNum, int pageSize, int renderingFlag, QObject *parent)
...
QByteArray HttpTaskListCmd::serializeParams()
{
    QJsonObject object;
...
void HttpTaskListCmd::analysisResponsed(const QByteArray& data)
{
    // QtConcurrent::run(this, &HttpTaskListCmd::analysisThread, data);
    this->analysisThread(data);
}

void HttpTaskListCmd::analysisThread(const QByteArray& data)
{
...
QList<MaxTaskItem *> HttpTaskListCmd::analysisRespList(const QJsonArray& jsonArr)
{
